...
app.get('/api/admin/comments/all', requireAdmin, async (c) => {
  ...
  const result = await db!.comments.getList({ url: '', page, pageSize, includeSpam })
  return c.json(result)
})
...
app.get('/api/admin/stats', requireAdmin, async (c) => {
  await initDb()
  // 统计评论数
  const allComments = await db!.comments.getList({ url: '', page: 1, pageSize: 1000, includeSpam: true })
