...
import type { Comment, User, Config, CreateCommentInput, UpdateCommentInput, CommentQuery, PaginatedResult, TursoConfig } from '../types'
import { DatabaseAdapter, type CommentRepository, type UserRepository, type ConfigRepository } from './base'
...
class TursoCommentRepository implements CommentRepository {
  ...
  async create(data: CreateCommentInput): Promise<Comment> {
    ...
    await this.client.execute({
      sql: `INSERT INTO comments (id, url, nick, mail, link, content, ua, ip, rid, pid, created_at) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [id, data.url, data.nick, data.mail ?? null, data.link ?? null, data.content, data.ua ?? null, data.ip ?? null, data.rid ?? null, data.pid ?? null, now]
    })
  ...
  async getList(query: CommentQuery): Promise<PaginatedResult<Comment>> {
    ...
    const countResult = await this.client.execute({ sql: countSql, args: countArgs })
    ...
    const listResult = await this.client.execute({ sql: listSql, args: listArgs })
...
export class TursoAdapter extends DatabaseAdapter {
  ...
  constructor(config: TursoConfig) {
    ...
    const url = config.url.startsWith('file:') || config.url.startsWith('./') || config.url.startsWith('/')
      ? config.url
      : config.url
